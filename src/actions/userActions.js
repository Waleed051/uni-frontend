...
export const login = (email, password) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(
      "https://sleepy-plains-69712.herokuapp.com/api/users/login",
      { email, password },
      config
    );
    dispatch({
...
export const register = (name, email, password) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.post(
      "https://sleepy-plains-69712.herokuapp.com/api/users",
      { name, email, password },
      config
    );
    dispatch({
...
export const getUserDetails = (id) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(
      `https://sleepy-plains-69712.herokuapp.com/api/users/${id}`,
      config
    );
    dispatch({
...
export const updateUserProfile = (user) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.put(
      `https://sleepy-plains-69712.herokuapp.com/api/users/profile`,
      user,
      config
    );
    dispatch({
